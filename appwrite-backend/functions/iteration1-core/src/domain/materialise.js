...
export function validatePattern(pattern) {
  ...
  if (!Number.isInteger(pattern.start_minute) || !Number.isInteger(pattern.end_minute) ||
      pattern.start_minute < 0 || pattern.end_minute > 1440 || pattern.end_minute <= pattern.start_minute) {
...
export function occurrenceId(patternId, dateLocal) {
  const digest = createHash("sha256").update(`${patternId}|${dateLocal}`).digest("hex").slice(0, 32);
  return `p_${digest}`;
}
